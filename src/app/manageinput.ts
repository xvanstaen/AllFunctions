  ...
  if ( i!==0){
    ...
    if ( myObj.type_error===0){
      ...
      if( i=== ref_format.MyDateFormat.length){
        ...
        if ( datePipeInput< datePipeToday){ // check inuput date with Today date
                // date is out of range
                 myObj.error_msg = "invalid date; cannot be in the past";
                 myObj.type_error=7;
        } else 
          {
              if ( datePipeInput>= datePipeMax){ // check inuput date with Max date
            // date is out of range
                 myObj.error_msg = "invalid date; cannot be more than one year in the future";
                 myObj.type_error=8;
            }
        }
      }
